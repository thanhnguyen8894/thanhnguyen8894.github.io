...
export const useAddressBookPage = (props = {}) => {
    ...
    const handleEditAddress = useCallback(address => {
        ...
        // custom District
        const { region_code } = address.region || {};
        const newAddress = {...address, district: region_code};
    ...
    const handleConfirmDialog = useCallback(
        async formValues => {
            ...
            payload.region = {
                region: formValues.district,
                region_code: formValues.district,
                region_id: 0
            };
            payload.default_shipping = customerAddresses &&
                customerAddresses.length === 0 ? true : formValues.default_shipping;
            payload.default_billing = formValues.default_shipping;
    ...
    const formErrors = useMemo(() => {
        if (displayError) {
            return new Map([
                ...
                ['deleteCustomerAddressMutation', deleteCustomerAddressError]
            ]);
        } else return new Map();
    },[
