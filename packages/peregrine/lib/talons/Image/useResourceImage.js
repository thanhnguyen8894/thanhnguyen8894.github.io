...
export const useResourceImage = props => {
    ...
    const srcSet = useMemo(() => {
        return generateSrcset(resource, type, ratio)?.includes('https://')
            ? generateSrcset(resource, type, ratio)
            : generateSrcsetWithBaseUrl(resource, type, ratio);
    }, [generateSrcset, resource, type, ratio, generateSrcsetWithBaseUrl]);
