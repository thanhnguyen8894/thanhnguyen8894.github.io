...
import useFieldState from '@magento/peregrine/lib/hooks/useInformedFieldStateWrapper.js';
...
export const useQuantity = props => {
    ...
    const handleDecrement = useCallback(() => {
        ...
        if (newQuantity <= qtySalable) {
            setShowError(false);
        }
    }, [debouncedOnChange, quantity, quantityFieldApi, qtySalable]);

    const handleIncrement = useCallback(() => {
        const newQuantity = quantity + 1;
        if (newQuantity > qtySalable) {
            quantityFieldApi.setValue(qtySalable);
            debouncedOnChange(qtySalable);
            setShowError(true);
        } else {
            quantityFieldApi.setValue(newQuantity);
            debouncedOnChange(newQuantity);
    ...
    const handleBlur = useCallback(() => {
        ...
        setIsCartUpdating(isNaN(quantity) || quantity == 0);
    }, [debouncedOnChange, prevQuantity, quantity]);
    const handleFocus = () => setIsCartUpdating(true);
