...
const flattenData = data => {
    if (!data) return {};
    return {
        subtotal: data.cart.prices.subtotal_including_tax,
...
export const usePriceSummary = props => {
    ...
    const handleProceedToCheckout = useCallback(() => {
        ...
        } else history.push('/checkout/registration');
    }, [history, isSignedIn]);

    return {
        handleProceedToCheckout,
        // only hasError when had errors and data == null or undefined
        hasError: !!error && !data,
        hasItems: data && !!data.cart.items.length,
        isCheckout,
        // disable button checkout when cart item still have errors
