...
export const useBreadcrumbs = props => {
    ...
    const convertPath = useMemo(() => {
        if (breadcrumbs && breadcrumbs.length > 0) {
            return breadcrumbs[breadcrumbs.length - 1].category_url_path.replace('.html','/');
        } else return '';
    },[breadcrumbs]);

    const currentPath = useMemo(() => {
        if (data?.category?.url_path?.includes(convertPath)) {
            return data?.category?.url_path
        } else return `${convertPath}${data?.category?.url_key}`;
    },[data, convertPath]);

    const categoryUrlSuffix = (data && data.category && data.category.url_suffix) || '';
    ...
    return {
        currentCategory: (data && data.category && data.category.name) || '',
        currentCategoryPath: (currentPath && `${currentPath}${categoryUrlSuffix}`) || '#',
        isLoading: loading,
        hasError: !!error,
