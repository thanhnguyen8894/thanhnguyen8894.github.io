 ...
 */
export const DEFAULT_WIDTH_TO_HEIGHT_RATIO = 1;

const baseMediaUrl = 'https://cdn-ara-images.halaexpress.com';
...
export const generateSrcsetWithBaseUrl = (imageURL, type, ratio) => {
    if (!imageURL || !type) return '';
    ...
    return Array.from(imageWidths, ([, value]) => value)
        .map(
            width =>
                `${baseMediaUrl}${generateSrcsetUrl(
                    width,
                    Math.round(width / imageRatio)
                )} ${width}w`
        )
