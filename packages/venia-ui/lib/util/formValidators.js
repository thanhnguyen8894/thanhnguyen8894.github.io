...
export const validatePhoneNumberSA = value => {
    const FAILURE = {
        id: 'validation.isRequired',
        defaultMessage: 'Is required.'
    };
    ...
    const reg = /^(009665|09665|9665|\+9665|05|5)(5|0|3|6|4|9|1|8|7)([0-9]{7})$/;
    const message = {
        id: 'validation.validatePhoneNumberSA',
        defaultMessage: 'Phone number is not valid'
    };
    return reg.test(value) === false ? message : SUCCESS;
};

export const validateDecimalNumber = value => {
    const message = {
        id: 'validation.validateDecimalNumber',
        defaultMessage: 'Please input valid amount (E.g: 1.55)'
    };
    if (
        typeof value === 'string' &&
        (value?.match(/,/g)?.length > 1 || value?.match(/\./g)?.length > 1)
    ) {
        return message;
    }
    return SUCCESS;
...
export const validatePhoneNumber = (value, country_code) => {
    const FAILURE = {
        id: 'validation.isRequired',
        defaultMessage: 'Is required.'
    };
    ...
    const validPhone = isValidPhoneNumber(stringValue, country_code) === true;

    return validPhone ? SUCCESS : message;
};
...
export const validateEmail = value => {
    ...
    if (typeof value === 'string' && !value.match(EMAIL_FORMAT)) {
        return message;
    }
    return SUCCESS;
};
