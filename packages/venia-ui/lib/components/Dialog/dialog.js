...
const Dialog = props => {
    ...
    let rootClass = isOpen ? classes.root_open : classes.root;
    if (rtl) {
        rootClass += ` ${classes.rootRtl}`;
    }
    ...
    const dialog = isPopup ? (
        <div
            className={`${classes.dialogPopup} ${
                mobile ? classes.dialogPopupMobile : ''
            }`}
        >
            <div className={classes.headerPopup}>
                <span className={classes.headerTextPopup}>{title}</span>
                {maybeCloseXButton}
            </div>
            <div className={classes.bodyPopup}>
                <div className={classes.contentsPopup}>{children}</div>
                {maybeButtons}
            </div>
        </div>
    ) : (
